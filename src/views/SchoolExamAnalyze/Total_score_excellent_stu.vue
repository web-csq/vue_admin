<template>
  <div>
    ...
    <div class="tab-container">
      <div class="tab-title">总分优秀学生班级报表
...
<script>
import { selectGradeListClassLevleOne } from "@/api/schoolAnalyze"
...
export default {
  ...
  methods:{
    downloadImg(){
      this.$downloadChart("d1","总分优秀学生班级报表")
    },
    exportData(){//导出全校排名数据
        this.$refs.table1.exportCsv({
          filename: this.examInfo.name +'总分优秀学生班级报表'
      });
    },
    setLineChart(data){
    ...
    async setSelectGradeListClassLevleOne(){//总分优秀学生班级分布图
      this.data = [];
      this.chartList = [];
      selectGradeListClassLevleOne({
  ...
  mounted(){
    this.setSelectGradeListClassLevleOne();
  }
}
</script>
