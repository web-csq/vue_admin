...
<script>
import { selectGradeListClassLevleThree } from "@/api/schoolAnalyze"
...
export default {
  ...
  methods:{
    downloadImg(){
      this.$downloadChart("d1","总分良好学生班级报表")
    },
    exportData(){//导出全校排名数据
        this.$refs.table1.exportCsv({
          filename: this.examInfo.name +'总分良好学生班级报表'
      });
    },
    setLineChart(data){
    ...
    async setSelectGradeListClassLevleThree(){//总分优秀学生班级分布图
      this.data = [];
      this.chartList = [];
      selectGradeListClassLevleThree({
  ...
  mounted(){
    this.setSelectGradeListClassLevleThree();
  }
}
</script>
